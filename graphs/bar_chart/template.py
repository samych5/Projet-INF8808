...
def make_text_steps(start_index: int = 1):
    ...
    for i, step_config in enumerate(STEPS_CONFIG):
        ...
        if i == TRANSITION_STEP_INDEX:
            steps.append(
                html.Div(
                    className="story-step barchart-step barchart-transition-step",
                    **{"data-step": str(step_index)},
                    children=[
                        html.Div(
                            className="text-card barchart-transition-card",
                            children=[
                                html.P(
                                    "Regardons maintenant les impacts en général.",
                                    className="text-card-paragraph barchart-transition-text",
                                ),
                            ],
                        )
                    ],
            ...
            steps.append(
                html.Div(
                    className="story-step barchart-step",
                    **{"data-step": str(step_index)},
                    children=[
                        html.Div(
                            className="text-card",
...
def make_section(df: DataFrame, start_index: int = 1):
    ...
    return make_section_layout(
        ...
        section_id="section-bar-chart",
        section_class="bar-chart-section",
        data_section="barchart",
        panel_class="bar-chart-panel",
    )