...
def make_text_steps(start_index: int = 1):
    ...
    for i, step_config in enumerate(STEPS_CONFIG):
        ...
        if i == TRANSITION_STEP_INDEX:
            steps.append(
                html.Div(
                    className="story-step scatter-step scatter-transition-step",
                    **{"data-step": str(step_index)},
                    children=[
                        html.Div(
                            className="text-card scatter-transition-card",
                            children=[
                                html.P(
                                    step_config.text,
                                    className="text-card-paragraph scatter-transition-text",
                                ),
                            ],
                        )
                    ],
